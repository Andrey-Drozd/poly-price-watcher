...
class PriceObservationPolicyTest {
    ...
    @Test
    void shouldAcceptObservationWhenItIsNewerThanCurrentState() {
        TrackedPrice previousPrice = trackedPrice("0.62", "2026-04-01T10:15:29Z", PriceSource.BEST_BID_ASK);
    ...
    @Test
    void shouldAcceptObservationWhenOrderBookHasHigherPriorityThanPriceChange() {
        Instant observedAt = Instant.parse("2026-04-01T10:15:30Z");
        TrackedPrice previousPrice = trackedPrice("0.62", observedAt.toString(), PriceSource.PRICE_CHANGE);

        boolean ignored = policy.shouldIgnore(previousPrice, PriceSource.ORDER_BOOK, observedAt);

        assertThat(ignored).isFalse();
    }

    @Test
    void shouldIgnoreObservationWhenOrderBookHasLowerPriorityThanBestBidAsk() {
        Instant observedAt = Instant.parse("2026-04-01T10:15:30Z");
        TrackedPrice previousPrice = trackedPrice("0.62", observedAt.toString(), PriceSource.BEST_BID_ASK);
